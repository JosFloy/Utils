...
```java
long time=System.currentTimeMillis();    //得到系统时间
final Calendar c=Calendar.getInstance(); //得到Calendar引用
c.setTimeInMillis(time);				 //把系统时间设置到Calendar中 ？为什么要设置，直接就能得到的系统时间？多此一举？
mHour=c.get(Calendar.HOUR);				 //得到系统的小时数
mMinute=c.get(Calendar.MINUTE);			 //得到当前时间中的分数
