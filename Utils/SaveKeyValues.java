...
/**
 * 用SharedPreferences来保存常用的属性(以Key-Values的形式)
 */
public class SaveKeyValues {
     ...
     * @return true未被创建,false被创建
     */
    private static boolean isUnCreate(){
        boolean result = sharedPreferences == null;
        if (result){
     ...
     * @return
     */
    public static void putIntValues(String key, int values){
        if (isUnCreate()){
            return;
        }
     ...
     * @return
     */
    public static void putBooleanValues(String key, boolean values){
        if (isUnCreate()){
            return;
        }
